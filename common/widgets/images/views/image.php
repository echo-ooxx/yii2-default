...
<div class="fileinput fileinput-new" data-provides="fileinput">
    ...
    <div class="fileinput-preview thumbnail" data-trigger="fileinput" style="width: 200px; height: 150px;">
        <img class="file_img" src="<?= !empty($picture['path']) ? $picture['path'] : \yii\helpers\Url::to('@web/widgets/images/no.png'); ?>">
    </div>
</div>
        ...
        if (/^image/.test( files[0].type)){
            ...
            reader.onloadend = function(){
                $.ajax({
                    ...
                    dataType: 'json',
                    beforeSend: function(){
                        file_img.parent('.thumbnail').css({
                            'background-image':'url("<?= \yii\helpers\Url::to('@web/widgets/images/_loading.gif')?>")'
                        });
                        file_img.attr('src','');
                    },
                    success: function(json){
                        if(json.boo){//console.log(file_ipt);console.log(file_img);
                            ...
                            console.log(json);
                            file_img.attr('src',json.data.url);
                            file_ipt.val(json.data.url);
                        } else {
                            alert(json.msg);
                        }
                    },
