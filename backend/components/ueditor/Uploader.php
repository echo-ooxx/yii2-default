...
//        $this->stateMap['ERROR_TYPE_NOT_ALLOWED'] = iconv('unicode', 'utf-8', $this->stateMap['ERROR_TYPE_NOT_ALLOWED']);
    ...
    private function upFile()
    {
        ...
        if($this->config['isAddWater'] == 1){
            //添加水印
            $disabled = [
                '.gif'
            ];
            if(!in_array($this->getFileExt(),$disabled)){
                $job = [
                    'watermark' => [
                        'mark' => Yii::$app->getBasePath() . '/components/ueditor/water.png',
                        'position' => 0
                    ]
                ];
                $image = new ImageFilter($file,$job,$file['tmp_name']);
                $image->outimage();
            }
        }
